...
// Set initial positions based on the figure-eight orbit
body1.position.set(-0.97000436, 0.24308753, 0);
body2.position.set(0.97000436, -0.24308753, 0);
...
let vel1 = new THREE.Vector3(0.466203685, 0.43236573, 0);
let vel2 = new THREE.Vector3(0.466203685, 0.43236573, 0);
let vel3 = new THREE.Vector3(-0.93240737, -0.86473146, 0);

// Initialize trails with matching colors
const trailLength = 1000;
const trailMaterial1 = new THREE.LineBasicMaterial({ color: 0xff0000 }); // Red
const trailMaterial2 = new THREE.LineBasicMaterial({ color: 0x00ff00 }); // Green
const trailMaterial3 = new THREE.LineBasicMaterial({ color: 0x0000ff }); // Blue

const positions1 = new Float32Array(trailLength * 3).fill(0);
const positions2 = new Float32Array(trailLength * 3).fill(0);
const positions3 = new Float32Array(trailLength * 3).fill(0);
...
function updatePositions() {
    ...
    for (let i = trailLength - 1; i > 0; i--) {
        positions1[i * 3 + 0] = positions1[(i - 1) * 3 + 0];
        positions1[i * 3 + 1] = positions1[(i - 1) * 3 + 1];
        positions1[i * 3 + 2] = positions1[(i - 1) * 3 + 2];
        positions2[i * 3 + 0] = positions2[(i - 1) * 3 + 0];
        positions2[i * 3 + 1] = positions2[(i - 1) * 3 + 1];
        positions2[i * 3 + 2] = positions2[(i - 1) * 3 + 2];
        positions3[i * 3 + 0] = positions3[(i - 1) * 3 + 0];
        positions3[i * 3 + 1] = positions3[(i - 1) * 3 + 1];
        positions3[i * 3 + 2] = positions3[(i - 1) * 3 + 2];
    }
    positions1[0] = pos1.x; positions1[1] = pos1.y; positions1[2] = pos1.z;
    positions2[0] = pos2.x; positions2[1] = pos2.y; positions2[2] = pos2.z;
    positions3[0] = pos3.x; positions3[1] = pos3.y; positions3[2] = pos3.z;
...
// Position camera for a better view of the simulation
